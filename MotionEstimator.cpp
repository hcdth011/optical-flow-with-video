...
void MotionEstimator::detectFeaturePoints()
{
    ...
    // Detect the features
    cv::goodFeaturesToTrack(grayPrev,       // input image ,注意，是上一张图片
...
void MotionEstimator::performFeatureDetection(Mat& frame)
{
    ...
    //光流的计算只需要找一次特征点，然后后面每一帧通过计算光流来追踪原来的特征点在哪
    //如果每次都去找新的特征点，计算会很慢 ，误差也会加大
    //同时，由于只计算了一次特征点，所以只允许摄像头在小范围内移动，摄像头视角要大
    if (addNewPoints()) //只有上一张图像的featrue少于10时才会检查新的featrue
    {
